...
- ❌ Campaign structure
- ❌ Different human defender classes (currently basic sentry only)
...
**Panic Spreading (v0.16.0, refined v0.21.2):**

When a human detects an ally in GRAPPLED state within 40px:
- Immediately switches to FLEEING state
- Triggers `propagate_flee_to_group()` — a depth-capped, distance-based panic wave

**Propagation rules (v0.21.2):**
- `panic_propagation_depth` (default: 2) limits how many hops the chain travels
- Depth 0: direct detector flees + propagates to neighbours
- Depth 1: neighbours flee + propagate one more ring
- Depth 2: outer ring flees — chain stops
- Reaction delay is distance-based: ally 80px away = 0.4s, ally 5px away ≈ 0.025s
  - Tight formations react near-simultaneously; loose groups get a visible outward wave
- 80px propagation radius per hop (not 40px — that's only the grapple trigger check)

**Formation spacing tactical impact:**
- Tight (≤80px): Whole contact group + 2 outer rings panic — significant cascade
- Loose (>160px): Only the immediate cluster affected — isolated reaction
