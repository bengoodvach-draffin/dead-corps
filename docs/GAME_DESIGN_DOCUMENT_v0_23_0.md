...
When a zombie has been in a human's vision cone for 5 seconds, the human alerts nearby allies within 150px. Each ally rotates to a class-appropriate facing offset based on which side of the alerter they're on — right-side allies rotate right, left-side allies rotate left. Multiple allies on the same side get increasing offsets so they spread out rather than clustering.

| Class | Response | Offsets |
|-------|----------|---------|
| Civilian | Flee (existing morale system) | — |
| Militia | All face threat directly | 0° |
| Police | Fan out around threat | ±45°, ±90° |
| GI | Cover flanks and further around | ±105°, ±165° |
| Spec Ops | Same as GI for now | ±105°, ±165° |

Gunshot response: when any human fires, nearby IDLE/SENTRY humans within 150px rotate to face the target after a 0.4s reaction delay. All rotations are smooth at 360°/sec.

**Tactical implications:**
- Player cannot park zombies in vision cones indefinitely — at 5 seconds the formation adapts
- Militia alert creates a predictable blind spot (all facing same direction) — exploitable
- GI alert covers flanks — requires more careful approach
- Gunshot collapses all nearby facing onto the active threat
...
- ✅ Smooth rotation at 360°/sec for both alert types

**Next Phase (First Validation Slice):**
- Integration testing of v0.22.x–v0.23.0 systems together
- Costume Zombie and Fat Zombie (first special types)
- One handcrafted test level with Civilian, Militia, Police, GI, Spec Ops and both zombie types
...
- Multiple same-side allies get increasing offsets (1st: 105°, 2nd: 165°) so no two units face the same direction
- Alert cooldown 30s; facing returns to original after 2 min of clear cone; patrol resumes after 30s
- Gunshot response: firing human broadcasts to nearby IDLE/SENTRY units (150px), 0.4s reaction delay, 1s per-human cooldown
