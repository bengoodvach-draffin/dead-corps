...
- ✅ **The perf sampler** — BUILT 2026-08-02 (`perf_sampler.gd`, GM-instantiated, gated by `GameConfig.perf_log`): one ⏱️ PERF line per 5s to the console/log — frame avg/worst, physics time, node count, unit census by state. Claude reads the curve from `godot.log`; no screenshots. Plus **`GameConfig.perf_auto_frenzy_delay`** (default 0 = off): N seconds after boot every calm zombie auto-ignites on a sim_tick timer, so a full-frenzy load test runs headless with nobody at the keyboard — this is also the vehicle for the §10 boot-twice diff. Test bed: `level_testing.tscn`.
- ✅ **The ground-up review — DONE 2026-08-02: read `docs/PERF_REVIEW.md`** (supersedes CODEBASE_REVIEW's perf deep-dive). Headline: the frenzy is fixed but the IDLE map costs ~14ms of the 16.7ms budget at 491 units — the fixed per-unit floor is the problem now. Ranked plan: **F2 idle LOD + F5 stationary-physics audit** (next perf round) → **F4 batched presentation = fold into Phase 5's vision_renderer rewrite** (one workstream, two goals) → **F1 components→RefCounted (needs Ben's ARCHITECTURE_GUIDELINES ruling)** → **F3 flow fields (parked; the 1000-zombie enabler, revisit past ~300 sustained ferals)**.
- ✅ **The §10 determinism check — PASSED 2026-08-02**: two headless auto-frenzy runs of level_testing, 119 sim-event lines each, byte-identical diff at wildly different render rates. The whole v0.47–48 cadence/grid stack is framerate-independent.
- ✅ **F2 idle LOD — BUILT + VERIFIED 2026-08-02** (the "cold human" model, `Human._lod_tick`): idle physics step 14 → 6–9 ms; the standard load test holds locked 60fps end to end; determinism re-verified F2-vs-F2 (full event streams identical). F5 withdrawn — its premise (stationary `move_and_slide` waste) was false on reading unit.gd. **Next perf step = F4, folded into Phase 5's vision_renderer rewrite.**
