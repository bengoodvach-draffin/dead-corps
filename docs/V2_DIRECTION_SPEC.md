...
- **Decay:** the front decays (~2× fill speed back toward zero) only when NO zombie is in range/LOS. While any threat is visible, the gun stays hot — it never decays and never resets except by firing.
