...
- ❌ Campaign structure
- ❌ Different human defender classes (currently basic sentry only) — **designed in v0.22.0 spec, implementation pending**
- ❌ Environmental hazards (water, fire, spikes)
- ❌ Sound and music
- ❌ Final art style

**NEW: Human Defender System (v0.22.0 — designed, not yet implemented):**
- ❌ Five defender classes: Civilian, Militia, Police, GI, Spec Ops
- ❌ Morale bar system replacing binary flee trigger and depth-capped propagation
- ❌ Shooting system with per-class weapons, aim time, and tracer line visuals
- ❌ Dual-zone vision arcs (detection zone + shooting zone)
- ❌ Tunnel Vision state for GI/Spec Ops (10s locked 45° cone)
- ❌ Zombie death visual (dark red color, shot knockback)
- ❌ Vision range increase: SENTRY/FLEEING arcs 180px → 350px
- ❌ Camera default zoom set to ~2.5× (Shadow Tactics scale)
...
Current implementation uses a depth-capped propagation chain (`propagate_flee_to_group()`, default 2 hops, 80px radius per hop). This is being deprecated and replaced by the morale system below.
...
- GI, Spec Ops → Tunnel Vision (10s, locked rotation, 45° cone)
...
- TUNNEL VISION state → Locked arc (45°, 350px range) — GI/Spec Ops only
...
Vision (Tunnel Vision): 350px arc, 45° angle, locked rotation, 10s duration
```
...
### 6.2 Current Prototype (Pre-v0.22.0)

**Basic Sentry:**
- Health: 75
- Speed: 90 (fleeing), 50 (patrolling)
- Behavior: Patrols waypoints OR stands watch
- Vision: Arc vision (180px range, 90° angle) — increasing to 350px in v0.22.0
- Swing arc: Optional (scans left/right while stationary)
- Panic spreading: Triggers when ally grappled within 40px (being replaced by morale system)
- Combat: None (doesn't fight back — shooting system coming in v0.22.0)
- Conversion: Becomes zombie on death

**Sentry States (current):**
...
**New States (v0.22.0):**
- TUNNEL_VISION: GI/Spec Ops — locked rotation, 45° cone, 10s duration
- FREEZE: Civilian only — 5s paralysis (deferred)
- MELEE_CHARGE: Militia only — rush attack (deferred)
...
- 2 hops × 80px radius per hop = ~160px effective spread from contact
...
- Costume Zombie and Fat Zombie (first special types)
- Police and GI defender classes
- One handcrafted test level with all four
...
- Tunnel Vision state for GI/Spec Ops: 10s locked rotation, 45° cone
- Vision range increase: SENTRY/FLEEING arcs 180px → 350px
- Zombie death visual: dark red color + shot knockback
- Camera default zoom set to ~2.5× (Shadow Tactics scale)
