...
**Version:** 6.1 (Advanced Prototype Build)  
**Last Updated:** April 21, 2026  
**Status:** Prototype v0.25.6
...
## 2. CURRENT PROTOTYPE STATE (v0.25.6)
...
**Alert System (v0.23.0–v0.25.6):**
- ✅ Low urgency detection alert: 5s cone timer, all classes face threat directly (v0.25.5 — per-class offsets removed), smooth rotation
...
- ✅ All return-to-original-facing rotations now smooth (v0.25.6 — was instant snap)
- ✅ `_pre_alert_facing` preserves true original facing across chained alerts (v0.25.2)
...
- ✅ Click-to-pin cone: left-click human shows their cone; click again to toggle off; click another to move pin
- ✅ V key shows all cones simultaneously (debug mode)
- ✅ Facing lines: white 20px indicator always drawn for every living human
- ✅ Tunnel vision cone always shown regardless of pin status
- ✅ Merged/group blob cone logic removed entirely
- ✅ BOID cohesion force disabled (was pulling units together for merged blobs)
- ✅ Clicking a human preserves zombie selection

**Shooting & Targeting Improvements (v0.25.2–v0.25.3):**
- ✅ Shoot target facing: armed sentries smoothly rotate toward active shoot target while aiming
- ✅ Swing suppressed while shoot tracking active; returns to original facing 2s after target lost
- ✅ Smart retargeting: switch to closer zombie entering weapon range with 50% aim time penalty
...
**Vision + Alert Polish (v0.25.1-v0.25.6):**
- Click-to-pin vision cone system; facing lines always shown (v0.25.1)
- Alert facing fixes: _pre_alert_facing, shoot target tracking, smooth return (v0.25.2)
- Smart retargeting with 50% aim penalty (v0.25.3)
- Debug logging pass (v0.25.4)
- Detection alert direct facing — per-class offsets removed (v0.25.5)
- Smooth return rotation; tunnel vision cone always shown (v0.25.6)
...
**Visual Rendering (v0.25.1):**
- No cones shown by default — click-to-pin system controls visibility
- **Facing lines:** short white line (20px) always drawn from every living human in their facing direction
- **Tunnel vision cone always shown** regardless of pin — the narrow orange arc is the primary signal to the player that a GI/Spec Ops has entered tunnel vision
- **Click-to-pin:** left-click a human to show their cone; click same human to toggle off; click another to move the pin
- **V key debug mode:** shows all cones simultaneously
- Clicking a human does not clear zombie selection
- Dual-zone arcs communicate threat range vs detection range at a glance
- Merged/group blob cone logic removed in v0.25.1
...
When a zombie has been in a human's vision cone for 5 seconds, the human alerts nearby allies within 150px. All classes face the threat directly — per-class formation offsets were removed in v0.25.5 after they caused a ping-pong bug when multiple alerters sent contradictory rotation targets to a shared ally.

| Class | Response |
|-------|----------|
| Civilian | Flee only — no facing assignment |
| Militia, Police, GI, Spec Ops | All face threat directly (0°) |

Note: Per-class offsets (Police ±45°/±90°, GI/Spec Ops ±105°/±165°) are preserved as commented-out reference in `human.gd` for potential future reimplementation.

Alert cooldown: 30 seconds. Facing returns to original 30 seconds after cone clears, using smooth rotation (v0.25.6).
...
**Tactical implications:**
- Player cannot park zombies in vision cones indefinitely — 5s detection alert causes nearby humans to face the threat
- All classes converging on the threat direction creates a readable response — and a blind spot directly behind them
...
- Armed humans track their shoot target with their facing — the player can read who is being aimed at from the facing line
...
**Vision + Alert Polish (v0.25.1-v0.25.6 Complete):**
- ✅ Click-to-pin vision cone system; facing lines; tunnel vision cone always shown (v0.25.1)
- ✅ Alert facing fixes: _pre_alert_facing, shoot target tracking (v0.25.2)
- ✅ Smart retargeting with 50% aim penalty (v0.25.3)
- ✅ Detection alert direct facing — per-class offsets removed (v0.25.5)
...
**v0.25.6 (April 21, 2026):**
- Smooth return-to-original-facing rotation — was instant snap, now smooth at 360°/sec
- `_is_returning_to_original` flag tells rotation block to clear alert state on completion
- Tunnel vision cone always shown in vision_renderer.gd regardless of pin status

**v0.25.5 (April 21, 2026):**
- Per-class detection alert facing offsets removed — all classes now face threat directly (0°)
- `ALERT_OFFSETS` dictionary preserved as commented-out reference with original values
- Right/left ally sorting logic removed from `_broadcast_detection_alert`
- Fixes ping-pong bug: two alerters targeting same ally now give identical rotation target

**v0.25.4 (April 21, 2026):**
- Non-essential print statements commented out across human.gd
- Rotation/alert logging added: 🔒 PRE-ALERT CAPTURED, 🔄 ROTATING, ↩️ return logs, ⚡ HIGH URGENCY
- No gameplay changes

**v0.25.3 (April 21, 2026):**
- Smart retargeting: armed humans switch to closer zombie if one enters weapon range while current target is outside weapon range
- 50% aim time penalty on retarget — punishes leapfrog without making it free to exploit
- New `_find_in_range_target()` helper in human.gd

**v0.25.2 (April 21, 2026):**
- `_pre_alert_facing` captures actual facing_direction at first alert; preserved across chained alerts; zeroed on return
- Both return-to-facing paths now use `_pre_alert_facing` instead of `sentry_facing_degrees`
- Detection alert facing return timer fixed: was 120s in _receive_detection_alert, corrected to 30s
- Shoot target facing: armed SENTRY humans smoothly rotate toward active shoot_target while aiming (360°/sec)
- Swing arc suppressed while shoot tracking active
- Return to original facing 2s after shoot target dies or leaves range

**v0.25.1 (April 21, 2026):**
- Click-to-pin vision cone: left-click human to show cone, click again to toggle off, click another to move pin
- V key debug mode: show all human cones simultaneously
- Facing lines: white 20px line drawn from every living human in facing direction, always shown
- Tunnel vision cones always shown regardless of pin
- Merged/group blob cone logic removed from vision_renderer.gd
- BOID cohesion force disabled in unit.gd (was pulling units together for merged blobs)
- SelectionManager: clicking a human now preserves zombie selection
...
### 11.16 Why Direct Facing for Detection Alerts? (v0.25.5)

**Decision:** All classes face the detected threat directly (0° offset). Per-class formation offsets removed.

**Original design:** Each class fanned out to different angles — Militia faced directly, Police fanned ±45°/±90°, GI/Spec Ops covered ±105°/±165°. The intent was for each alerter to create a formation coverage response.

**Problem found during testing:** When two nearby sentries both fired detection alerts at roughly the same time, a shared ally between them received two contradictory rotation targets in quick succession and ping-ponged between them. The multi-alerter scenario is common in any grouped patrol setup.

**Why direct facing is better:**
- Two alerters pointing at the same zombie give the same rotation target — no conflict
- Readable: all nearby humans clearly look toward the threat
- Tactically meaningful: creates a visible "they've spotted something" moment with a clear blind spot behind them
- Class differentiation still lives in who responds (morale thresholds, tunnel vision) rather than where they face

**Preserved:** The original `ALERT_OFFSETS` dictionary is kept as commented-out reference in `human.gd` with all original values documented.

### 11.17 Why Smooth Return Rotation? (v0.25.6)

**Decision:** Return-to-original-facing after alerts uses smooth rotation at 360°/sec, not an instant snap.

**Problem:** The original code assigned `facing_direction = _pre_alert_facing` directly, which caused a jarring visual flick — particularly noticeable after the 2s high urgency hold expired.

**Fix:** `_is_returning_to_original` flag routes the return through the existing `_target_facing` rotation block, which already handles smooth rotation. Alert state (`_is_alerted`, `_pre_alert_facing`) is cleared by the rotation block when it completes, rather than immediately. This means swing suppression stays active during the return rotation, which is correct behaviour.
...
**Last Major Update:** v0.25.6 — vision click system, alert facing polish, smooth return rotation, shoot target tracking
