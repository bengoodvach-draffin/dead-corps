...
- **`IMPLEMENTER_GUIDE.md` status:** Spec 1 (spatial index) survives as the post-PoC reference for upgrading the Phase-0 internals; Spec 0 (benchmark) survives re-targeted at v2. Specs 3–5 and the invariants checklist are v1-bound (melee gate, corpse linger, morale, health bars) and need a v2 revision before any lower-capability-model execution on this branch. Do not hand the current guide to a cheap model against v2 code.
