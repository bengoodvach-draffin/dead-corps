...
**Progress (build plan `V2_POC_BUILD_PLAN.md`):** Phase 0 (foundations) ✅ and Phase 1 (demolition) ✅ are **complete** — the branch is the sterile sandbox (units move/patrol, no combat), RNG-free, with the registry + GameConfig + EscapeBarrier in place. **Next: Phase 2 — predation core** (2.1 idle shamble → 2.2 FERAL + the Pounce → 2.3 retarget/hunt pool → 2.4 release → 2.5 contagion → 2.6 risers), then Phase 3 (defense: fill front, fear/break, rout, cower), Phase 4 (win/lose + combo), Phase 5 (readability), Phase 6 (PoC level + M1 playtest + the **sacred-ratio sweep** until a GI kills ~3–4 of a charging wave), Phase 7 (the Mark + M2). The spec's seven validation questions (§12) are the verdict criteria. Propose-before-implementing applies to every step.
