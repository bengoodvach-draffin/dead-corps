...
This describes the codebase **as it exists now on `v2-poc` (v0.47.0) — the M1 core loop feature-complete, plus enterable buildings slice 1 + the terrain kit (v0.44.0, buildings spec §16), the Mark (v0.45.0, M2's first half — behind the C key, pending a design workshop), calm door-breaching + the door-click grammar change (v0.46.0), and the flee exit rules (v0.47.0).** The v1 stealth systems (morale, alerts, vision cones, tunnel vision, HP/grapple melee, leap, the incubation pipeline, Spec Ops) are **gone**; built in their place: **predation** (feral/Pounce/contagion/risers), **defense** (fill front/fear break/rout+herding/cower), **shelters & sieges** (SHELTERED/BREACHING/breach/flush), **win/lose** (§8), **combo scoring** (§6), and **nav pathing** for both zombies and humans. Still to come: Phase 5 (readability `vision_renderer` rewrite), Phase 6 (PoC level + sacred-ratio sweep + M1 verdict), LMB fill-inspect (M2's other half). The codebase is **fully RNG-free** (spec §10) and all neighbour/unit lookups go through the GameManager registry. **Structure = dispatcher-shell + behavior-component pattern** (ARCHITECTURE_GUIDELINES): `Zombie`/`Human` are thin shells whose per-frame dispatch routes to child behavior-component nodes (one mechanic per component).

**Phase 0 foundations (the v2 substrate):**
- `game_config.gd` (autoload `GameConfig`) — single source of truth for every spec-§9 tunable. `level_config.gd` (`@tool LevelConfig`) optionally overrides per level. Nothing hardcodes a §9 number. **Defaults are no longer the spec's untested v0.1 numbers:** ten knobs were rebased onto Ben's hand-tuned `level_docks` values on 2026-07-30 (speed 140, fire cooldowns +0.25s, repel 3.0, exit-threat bias 6.0, scan 150, off-axis 1.5, cower window 1.2, release cluster 150 / aim 50, mark 300), each annotated with its old value. LevelConfig's `@export` defaults mirror them, so a `.tscn` that never set a knob now follows the docks baseline.
...
- `zombie.gd` (`Zombie` extends `Unit`) — player-controlled **dispatcher shell**. State {CALM, FERAL, DEAD}; `_physics_process` routes to `_tick_calm` (idle-shamble or commanded move) / `_tick_feral` (autonomous hunt — released-is-released, not selectable/commandable). **Nav-pathed** movement: `nav_move_toward` drives the `NavigationAgent2D` around obstacles for calm moves + feral pursuit (pounce flight stays straight). Components: `ShambleBehavior` (calm idle wander), `FeralBrain` (pursuit / retarget / continuous peel-off via `FeralTargeting` path-score), `PounceBehavior` (lunge → kill-at-landing → recovery; `abort()` releases the victim's claim on a mid-pounce death), **`CalmBreach`** (calm auto-breach, v0.46.0 — see below). `ignite_feral(target=null)` (release seed, or null = contagion self-target). `zombie_killed_human` signal. `is_special` for the PoC-excluded specials.
- **CALM DOOR-BREACHING (v0.46.0, Ben's rulings 2026-07-29/30)** — `calm_breach.gd` (`CalmBreach`, a Zombie component) + `door_breach.gd` (`DoorBreach`, static: the arc lookup, the shared pound stagger, and the reusable `Wedge` progress bucket — a static utility because components may not call each other). A CALM zombie breaks a door **without leaving calm control**, two ways in: *incidental* (an ordered move gains no ground for `door_wedge_window` while it sits in an intact door's arc) and *ordered* (`Zombie.order_breach` — RMB on the door). It stays selected and commandable; any new order cancels instantly. **Grammar rule this established: RELEASE IS FOR PREY, ORDERS ARE FOR TERRAIN** — a human or occupied building ignites the horde, a door or bare ground is commanded. Release-on-door (the old feral verb) was **deleted** 2026-07-30. `door_wedge_window` (0.5s) is deliberately split from `failsafe_window` (2s): the give-up clock must tolerate a slow chase, a door should convert to pounding at once.
- `human.gd` (`Human` extends `Unit`, `@tool`) — AI defender **dispatcher shell**. States IDLE/SENTRY/**FLEEING**/**SHELTERED**/**COWER**/DEAD; `defender_class` (CIV/MIL/POL/GI — indexes GameConfig per-class arrays); patrol (LOOP/PING_PONG); armed classes stamped with a class letter (M/P/G). Components: `FillBehavior` (the fill front — armed shot mechanic with **humans blocking the shot LOS**, per-class `fire_cooldown` gating the shot, the **door-watch** pre-aim while sheltered; + the civilian reaction-clock variant), `FearDetector` (fear-radius break, **building-LOS gated**; suspended while SHELTERED, sight-only uncapped inside a breached shelter), `FleeBehavior` (permanent rout — **nav-pathed** to the best exit from the **unified exit set** (zones ∪ intact shelter doors), **herding** flee-vector bend around zombies, shelter entry + the two-leg breach flush, + the cower net-displacement detector). **Exit choice (v0.47.0) is three passes:** availability → the **zombie-at-the-door filter** (`exit_block_radius`, 120px — an exit with a living zombie in it is struck off, so walling a door with bodies genuinely closes it; everything blocked → unfiltered fallback rather than freezing) → threat-biased distance score. `flee_exit_threat_bias` stays **6.0 on purpose**: it governs the ROUTE (humans were sprinting through the horde to reach a shelter behind it), the block radius governs the DOOR — neither substitutes for the other. Plus the **no-return latch**: a flushed runner that commits to a street exit writes off every shelter inside the building it was flushed from, permanently and for that human only (the outermost breached footprint when nested), and written-off doors can no longer absorb it on the walk past. `start_fleeing()` / `start_cowering()` / `cancel_fill()` / `enter_shelter()` / `is_safely_sheltered()`; `was_cowering` flags the terror bonus.
...
- **The buildings kit** (enterable buildings slice 1 + terrain kit — `V2_ENTERABLE_BUILDINGS_SPEC.md`, as-built §16): `shelter_building.gd` (`ShelterBuilding` extends `Polygon2D`, `@tool` — mouse-drawn footprint, auto wall quads minus door gaps, merged nav outlines, occupancy/spot claims, `is_shelter` flag for dumb boxes, scale self-heal), `door.gd` (`Door` — zombie barrier layer 2/8 + DoorLOS layer 5/16 + DoorLock layer 6/32; lock predicate, integrity/pounds/breach, inside-burst, `starts_open` portals, per-door `integrity_override`/`thickness`; standalone in a Wall gap = a gate), `shelter_spot.gd` (`ShelterSpot`, `is_guard` variant). **ShelterSpots are for RUNTIME ENTRANTS ONLY** (2026-07-30): a human PLACED inside a shelter is adopted as an occupant at boot but **holds exactly where the designer put it** (`add_occupant` — occupancy without a spot claim), because authoring a spot per resident was busywork at dozens-of-humans scale. Authored spots therefore stay free for humans who genuinely flee in later. Zombie side: **FERAL BREACHING lives in `FeralBrain`** — two ways in, both with a prey basis: building proxy (prey sheltered / release-at-the-building → own nearest door) and the **wedge** siege (prey beyond a barred door; the target is KEPT so the chase resumes the instant it falls, converting on the short `door_wedge_window`). Peel stays live, pour-in at breach, Mark siege-pull. There is **no ordered feral siege** — a door with nothing behind it is a job for the calm crew (`CalmBreach`). Collision masks: zombies 9, humans 33; LOS rays 17 (+humans 21 for fills).
...
**➡️ For the current prioritized to-do, read `docs/WORK_QUEUE.md`** (mirrored in the `v2-work-queue` memory). As of 2026-07-30: the ruled fix batches, playtest features, Tier-4 housekeeping, **enterable buildings slice 1 + the terrain kit (v0.44.0)**, the manager splits, **the Mark (v0.45.0)**, and the **2026-07-29/30 breach + flee batch (v0.45.3–v0.47.0)** are all DONE — what remains is Phase 5 (readability) → Phase 6 (PoC level + sweep + M1 verdict) → the Mark design workshop + LMB fill-inspect (M2). Buildings slice 2 (armory/barricade) stays parked until slice 1 plays at level scale.

**Design intent that the code implements but does not justify — do NOT "fix" these:**
- **Flushed humans burrowing deeper is INTENDED** (Ben, 2026-07-30). A breached market's occupants flee to the nearest safe zone, and an interior shop beats a distant street exit nearly every time, so they hide inward rather than running out. The cost is accepted: each hide is another door to break, so a nested market plays as a chain of sieges. Ben controls that through **level design** (limiting nesting depth), not engine rules. Removing it was proposed twice and declined twice.
- **`flee_exit_threat_bias` is high (6.0) on purpose** — it stops humans sprinting *through* the horde to reach a shelter behind it. It is not redundant with `exit_block_radius`; they address the route and the door respectively.
