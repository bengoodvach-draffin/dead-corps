...
**Current version:** v0.34.0 · **Engine:** Godot 4.6 / GDScript · **Perspective:** 2D isometric (full 3D migration still confirmed, but now waits behind the v2 pivot PoC)
...
**Progress (build plan `V2_POC_BUILD_PLAN.md`):** Phase 0 (foundations) ✅ and Phase 1 (demolition) ✅ complete — the branch is the sterile sandbox, RNG-free, with the registry + GameConfig + EscapeBarrier in place. **Phase 2 (predation core) underway:** 2.1 idle shamble ✅ (`ShambleBehavior`; established the zombie component architecture — `Zombie` is a dispatcher shell with `State {CALM, FERAL, DEAD}`, behaviors in child components per ARCHITECTURE_GUIDELINES rule 2). 2.2 FERAL + the Pounce ✅ (`FeralBrain` pursuit + `PounceBehavior` kill-at-landing/recovery; RMB-on-human releases selected zombies feral at the clicked human; pounce-exclusion via a Human-side claim; feral pursuit + calm moves both at `zombie_speed`). 2.3 retarget + hunt pool ✅ (on a kill/loss the feral chains to the nearest of a 250px LOS local-scan ∪ the GameManager-tracked pursued/fleeing hunt pool, excluding pounce-claimed; no-progress failsafe → calm). 2.4 release proper ✅ + playtested (cluster seeding via `ReleaseSeeder`, recall = calm-only, hover/hunted readability rings on the Human, hover misclick-defense). **Feral targeting reworked to the "peel-off + movement-vector" model** (the session's big design call, pillar-checked vs *bias-not-command / frenzy-eats-nearest*): the release is a *shove* that aims the horde; ferals then eat opportunistically — continuous divert onto the best **fresh, un-pursued** straggler (the pursuit-claim makes peel one-zombie-per-straggler so the bulk keeps momentum), never targeting humans *behind* their heading, and ranking targets by an along-the-path "bullet" score (`FeralTargeting.path_score`, shared by seeding + pursuit) so the horde drives up the centre and splays only as the central column clears. New knobs: `feral_divert_interval`/`_hysteresis`/`_offaxis_penalty`. **Next: 2.5 contagion → 2.6 risers.** (Flagged: zombie pursuit has no nav pathing — straight-line `step_toward` gets stuck on obstacles; the failsafe→calm is a stopgap; re-wire before the Phase 6 level. Also: spec §5.2/§3.4 now lag the implemented targeting model — sync when greenlit.) Then Phase 3 (defense: fill front, fear/break, rout, cower), Phase 4 (win/lose + combo), Phase 5 (readability), Phase 6 (PoC level + M1 + the **sacred-ratio sweep**), Phase 7 (the Mark + M2). The spec's seven validation questions (§12) are the verdict. Propose-before-implementing applies to every step.
